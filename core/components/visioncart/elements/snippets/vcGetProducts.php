...
$scriptProperties['extraFieldSortBy'] = $modx->getOption('extraFieldSortBy', $scriptProperties, false);
...
foreach ($scriptProperties as $parameter => $property) {
    if (substr($parameter, 0, 3) == 'tpl' && !in_array($parameter, array('tpl', 'tplOdd', 'tplFirst', 'tplLast'))) {
        ...
        $scriptProperties['tplNth'][] = array('nth' => (int)substr($parameter, 3), 'tpl' => $property);
...
for ($inc = 0; $inc < count($products); $inc++) {
    ...
    if ($inc == 0) {
        $output['master'] .= $vc->parseChunk($scriptProperties['tplFirst'], array_merge($products[$inc], array('count' => $inc)));
        continue;
    } else
        if ($inc == count($products)) {
            $output['master'] .= $vc->parseChunk($scriptProperties['tplLast'], array_merge($products[$inc], array('count' => $inc)));
            continue;
        }
    ...
    if (isset($scriptProperties['tplNth']) && !empty($scriptProperties['tplNth'])) {
        ...
        foreach ($scriptProperties['tplNth'] as $tpl) {
            if ($tpl['nth'] == $inc) {
                $output['master'] .= $vc->parseChunk($tpl['tpl'], array_merge($products[$inc], array('count' => $inc)));
                $break = true;
                break;
            }
        }
    ...
    if ($inc % 2) {
        $output['master'] .= $vc->parseChunk($scriptProperties['tplOdd'], array_merge($products[$inc], array('count' => $inc)));
        continue;
    }

    $output['master'] .= $vc->parseChunk($scriptProperties['tpl'], array_merge($products[$inc], array('count' => $inc)));
}
